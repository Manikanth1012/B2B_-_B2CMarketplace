...
export interface KbArticle {
  ...
  tags: string[]
  summary: string
  /* Ordered blocks. Was `[heading, prose]` pairs, which could not carry a third
     thing without every reader knowing what position meant what. */
  body: KbBlock[]
  status: KbStatus
  sort_order: number
}
...
/**
 * A block of an article.
 *
 * Three kinds, because three is what an article does: it says something, it
 * shows you the screen, or it plays you the walkthrough. The prototype had only
 * the first, so every article in this marketplace was words about a user
 * interface with no picture of it.
 *
 * A discriminated union rather than optional fields on one shape. `kind` is
 * what a renderer switches on, and a block carrying both `text` and `url` would
 * be a block nobody could say the meaning of.
 */
export type KbBlockKind = 'prose' | 'image' | 'video'

export interface KbProse { kind: 'prose'; heading: string; text: string }
export interface KbImage {
  kind: 'image'; heading: string; src: string
  /* Not optional. On a help page the reader most likely to need the picture
     described is the one who cannot see it, and a screenshot with no alt text
     is an instruction that does not exist for them. */
  alt: string
  caption?: string
}
export interface KbVideo { kind: 'video'; heading: string; url: string; caption?: string }
export type KbBlock = KbProse | KbImage | KbVideo

/**
 * The hosts this marketplace will put inside an iframe.
 *
 * A URL an author pastes becomes a frame on a page every persona reads, so an
 * open list is not a knowledge-base feature but a hole. Matched on the whole
 * host: a check written as "ends with youtube.com" also accepts
 * notyoutube.com, which is the entire trick.
 *
 * The database evaluates the same rule in `kb_embeddable`. This one is so the
 * editor can refuse before it saves.
 */
export const EMBED_HOSTS = ['www.youtube.com', 'youtu.be', 'player.vimeo.com', 'vimeo.com'] as const

export function embeddable(url: string): boolean {
  try {
    const u = new URL(url)
    return u.protocol === 'https:' && (EMBED_HOSTS as readonly string[]).includes(u.host)
  } catch { return false }
}

/**
 * The URL that actually goes in the frame.
 *
 * A person pastes what is in their address bar, which for both of these is a
 * watch page and not a player. Asking an author to find the embed URL is asking
 * them to do a conversion the software can do, and the one they will get wrong.
 * Returns null for anything not embeddable, so a caller cannot render one by
 * accident.
 */
export function embedUrl(url: string): string | null {
  if (!embeddable(url)) return null
  const u = new URL(url)
  if (u.host === 'youtu.be') return `https://www.youtube.com/embed${u.pathname}`
  if (u.host === 'www.youtube.com') {
    const v = u.searchParams.get('v')
    if (v) return `https://www.youtube.com/embed/${v}`
    return u.pathname.startsWith('/embed/') ? u.toString() : null
  }
  if (u.host === 'vimeo.com') return `https://player.vimeo.com/video${u.pathname}`
  return u.toString()
}

/** What is wrong with a block, in the sentence its author should read. */
export function blockProblem(b: KbBlock): string | null {
  if (!b.heading?.trim()) {
    return 'Every block needs a heading — it is what a reader scans for.'
  }
  if (b.kind === 'prose') {
    return b.text?.trim() ? null : 'A prose block with no words is a heading over a gap.'
  }
  if (b.kind === 'image') {
    if (!b.src?.trim()) return 'An image block needs a picture.'
    if (!b.alt?.trim()) {
      return 'An image needs alt text. The reader most likely to need the description is '
        + 'the one who cannot see the picture.'
    }
    return null
  }
  if (!embeddable(b.url ?? '')) {
    return `${b.url?.trim() || '(nothing)'} is not a host this marketplace will frame. `
      + 'Videos may be embedded from YouTube or Vimeo; anything else is linked as an attachment instead.'
  }
  return null
}

/** The first problem in an article's body, so a save can refuse with a reason. */
export function bodyProblem(body: readonly KbBlock[]): string | null {
  if (body.length === 0) return 'An article with no blocks is a title.'
  for (const b of body) {
    const why = blockProblem(b)
    if (why) return why
  }
  return null
}
...
/**
 * A body read tolerantly.
 *
 * The pairs are gone from the database, but a fixture, an export or a row
 * written before the migration is still a pair — and a reader that renders
 * nothing for one is a reader that silently drops an article's content.
 */
export function blocksOf(body: unknown): KbBlock[] {
  if (!Array.isArray(body)) return []
  return body.map(b => {
    if (Array.isArray(b)) {
      return { kind: 'prose' as const, heading: String(b[0] ?? ''), text: String(b[1] ?? '') }
    }
    return b as KbBlock
  }).filter(b => b && typeof b === 'object')
}

/** The blank an author starts a new block from. */
export function blankBlock(kind: KbBlockKind): KbBlock {
  if (kind === 'image') return { kind, heading: '', src: '', alt: '' }
  if (kind === 'video') return { kind, heading: '', url: '' }
  return { kind: 'prose', heading: '', text: '' }
}

export const BLOCK_LABEL: Record<KbBlockKind, string> = {
  prose: 'Words', image: 'A picture', video: 'A video',
}
