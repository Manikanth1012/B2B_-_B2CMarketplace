...
**Who chose what.** The operator can see every preference a recipient has set, because "why was I not told?" cannot be answered otherwise, and cannot change one, because that would be making the choice on somebody's behalf. Requirements: every preference resolves to a **named recipient** — a person with their account reference and role, or a seller account by its trading name — drawn from one directory across all four personas rather than rendered as the user id it is keyed on; a preference that covers a whole seller account says so, because a colleague still receiving the message is not a contradiction; and a preference whose owner the directory cannot name is reported as unresolvable rather than shown as a truncated identifier.
...
- **Providers are registered per channel** with protocol (SMPP 3.4, REST, SMTP/API, Graph, FCM HTTP v1), sender identity, throughput, measured success rate, region and whether the channel returns a delivery receipt at all.
- **A provider name is not an integration.** Each channel carries the record a real send needs: the endpoint and port, the authentication mode and a credential, the sender's registration with whichever authority the market demands (DLT in India, a sender-ID application in Kenya), the URL delivery receipts are posted back to, the timeout, the retry count and backoff, and the channel it falls over to. A channel without them can be switched on and has nothing to switch on.
- **The credential goes in and does not come back.** It is stored hashed; the record holds the last four characters so a desk can tell which key is loaded and the date it was set so a rotation policy has something to measure. There is no reveal, and the hash is not readable by any client — the column is outside the grant, not merely absent from the query.
- **Configured and verified are different claims.** Configured means every field a send needs is filled in. Verified means somebody ran the check and it passed. A console that only has *enabled* makes the stronger claim with the evidence for neither.
- **A check names what would fail rather than reporting a colour.** Missing endpoint, missing credential, receipts claimed with no callback to receive them, an unregistered sender, no rate on file — each is named, and a pass lists what it actually checked. A channel that claims delivery receipts and has nowhere for them to arrive will report delivery of messages nobody got, so the check refuses it.
- **Primary and failover are distinguished**, and failover is automatic after a defined number of attempts. The target must be an enabled channel of the same kind, must not be the channel itself, and must not point back — the first sends the retry where it cannot go and the last retries until the queue gives up.
...
- **What a message costs is a rate card, not a number.** A carrier quotes per destination, in the currency it bills in, effective-dated — Route Mobile bills Indian termination in rupees and Kenyan termination in shillings, and SES bills in dollars. Rates carry a segment size where the carrier bills by length, so a 300-character SMS is priced as the two segments it is charged as. A destination with no separate quote falls back to the carrier's default rate and the screen says that it did.
- **A rate is replaced, not edited.** Changing a price ends the old rate and opens a new one, so a message sent last month still reconciles against the rate that was live when it went.
- **A channel nobody has priced is unpriced, not free.** Zero is a rate — right for push and wrong for anything else — so the two are held apart and the check refuses a channel with neither.
- **Every message is priced by the platform from the rate card at the moment it is written**, never by whatever wrote the row, and carries its destination, its segment count and the currency it was billed in.
- **Spend is reported per currency and is never summed across them.** Adding rupees to shillings produces a figure that is not money anywhere, and putting a currency symbol in front of it makes the claim worse rather than better.
- Stale push tokens from uninstalled apps are separated out rather than folded into a failure rate, because they cost nothing and are not worth chasing.
...
| **NTF** — Notifications | **Demonstrated** | Rule builder, role-addressed rules, per-channel message templates with token preview and SMS segment counting, a named recipient behind every preference (§4.14), plus the **transport layer — providers with protocol and integration record, hashed credentials, sender registration, receipt callbacks, retry policy and guarded failover, a check that names what would fail, per-destination and per-segment rate cards in the carrier's own currency, DLR state machine, hard-rejection rules and per-currency channel spend** (§4.30). No message physically leaves the browser, and the check opens no socket — it refuses to call a half-configured channel healthy. |
