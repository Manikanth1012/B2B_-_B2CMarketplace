...
| Version | Date | Change |
|---|---|---|
| **4.5** | **Aug 2026** | **Published the TM Forum specifications themselves.** The seven Swagger/OpenAPI files from the 6D reference library ship with the app and are downloadable byte-for-byte; the reference renders every operation each declares (146 across the seven) marked live-here or spec-only, with a coverage figure. Versions now come from the files, moving five APIs to 4.0 or above. Inventory's TMF685 corrected to TMF687 Stock Management. Settlement and Party publish TM Forum's own TMF678 CustomerBill 5.0.0 and TMF632 Party Management 5.0.0, so every published API carries the standard it names at 4.0 or above. |
