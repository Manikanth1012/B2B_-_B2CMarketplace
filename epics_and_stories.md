...
| **APG** | Demonstrated | **Outbound**: seven APIs across sandbox and production, entitlement tiers with no pricing, reference docs with idempotency rules. **Inbound**: registry of every partner's endpoints, 23 marketplace events, coverage matrix, callback policy, test calls, call log. Integration is a **tested gate** that blocks go-live | Live gateway enforcement |
