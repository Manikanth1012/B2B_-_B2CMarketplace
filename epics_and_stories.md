...
>
> **v1.37 change**: the gateway behind the name. Six channels were "enabled" and none was integrated — a transport name and a protocol string, and no host, credential, sender registration, receipt callback, retry policy or failover target anywhere, while the screen promised failover "after a defined number of attempts" that nothing defined. `channel_integration`, `channel_rate` and `channel_test` added, with a check that names each part that would make a real send fail rather than reporting a colour, and two channels seeded deliberately broken so the console shows what unhealthy looks like. Message cost becomes a per-destination, per-segment, effective-dated rate card in each carrier's own currency; the flat `unit_cost` column is dropped rather than left to disagree with it, and spend is never summed across currencies again. "Who chose what" names its recipients instead of listing `e5b3c7a1…`. Two bugs found on the way: the credential hash was being handed to the browser by `select('*')`, now outside the grant; and the gateway read omitted `has_receipt`, so every row claimed no receipts and one of the two broken channels went unreported. 3,134 automated checks across two suites.
...
| NTF-FE-001-03 | Who Chose What, By Name **[P]** *(added v1.37)* | As a **support agent**, I want to answer "why was I not told?" without looking up a user id. | ✅ Every preference on file resolves to a named recipient — a person with their account reference and role, or a seller account by its trading name — from one directory spanning all four personas <br> ✅ A preference covering a whole seller account says so, because a colleague still receiving the message is not a contradiction <br> ✅ A preference whose owner cannot be named is reported as unresolvable rather than shown as a truncated identifier <br> ✅ The view is read-only: the marketplace can see what somebody chose and cannot choose for them | P1 | 5 |
...
| Story ID | Story Title | User Story | Acceptance Criteria | Priority | SP |
|----------|-------------|------------|---------------------|----------|----|
| NTF-BE-003-01 | Provider Registry with Failover **[P]** | As an **operator**, I want a standby for every channel that matters. | ✅ Provider carries channel, protocol, sender, throughput, region, measured success and receipt support <br> ✅ Primary and failover distinguished <br> ✅ A failover target must be an enabled channel of the same kind, must not be the channel itself and must not point back — the first sends the retry where it cannot go and the last retries until the queue gives up <br> ✅ A degraded provider is declared with the reason | P1 | 8 |
| NTF-BE-003-05 | The Integration Behind the Name **[P]** *(added v1.37)* | As an **operator**, I do not want a channel that can be switched on with nothing to switch on. | ✅ Each channel carries endpoint, port, authentication mode, credential, sender registry and reference, delivery-receipt callback, timeout, retry count, backoff and failover target <br> ✅ The credential is stored hashed and never returned — the record holds its last four characters and the date it was set, and the hash is outside the client's grant rather than merely absent from the query <br> ✅ **Configured** (every field a send needs is present) and **verified** (somebody proved it) are separate states with separate words <br> ✅ A channel that claims delivery receipts with no callback to receive them is refused, because it would report delivery of messages nobody got | P1 | 13 |
| NTF-BE-003-06 | A Check That Checks Something **[P]** *(added v1.37)* | As an **operator**, I want a health indicator that is evidence rather than a colour. | ✅ Every part that would make a real send fail is checked and named: missing endpoint, missing credential, receipts with no callback, unregistered sender, no rate on file <br> ✅ A pass lists what it actually checked, not that it passed <br> ✅ Every run is recorded with who ran it, when, the result and the findings <br> ✅ An enabled channel that cannot send is reported as such — it is live, and every message routed to it is lost | P1 | 8 |
| NTF-BE-003-07 | Message Cost as a Rate Card **[P]** *(added v1.37)* | As an **operator**, I want a message bill I can reconcile against a carrier invoice. | ✅ A rate is per channel and per destination, in the currency the carrier bills in, effective-dated, with a segment size where the carrier bills by length <br> ✅ A 300-character SMS is priced as the two segments it is charged as <br> ✅ A destination with no separate quote falls back to the default rate and the fallback is stated <br> ✅ Changing a price ends the old rate and opens a new one, so last month's messages still reconcile against last month's rate <br> ✅ **A channel nobody has priced is unpriced, not free** — zero is a rate, right for push and wrong for anything else <br> ✅ Every logged message is priced by the platform from the card at write time and carries its destination, segment count and billing currency | P1 | 13 |
...
| NTF-FE-003-02 | Channel Cost and Volume **[P]** | As an **operator**, I want the cost of talking to customers. | ✅ Sent, undelivered, rate and spend per channel, counted in **segments** because that is what a carrier bills <br> ✅ Channels with no receipt show *Not measured* rather than a rate <br> ✅ **Spend is reported per currency and never summed across them** — adding rupees to shillings produces a figure that is not money anywhere, and a currency symbol in front of it makes the claim worse | P2 | 5 |
| NTF-FE-003-04 | Configuring a Gateway **[P]** *(added v1.37)* | As an **operator**, I want to wire a carrier up and prove it works without leaving the console. | ✅ One form for address, authentication, sender registration, receipt callback, retry policy and failover, refusing what the database would refuse and offering only failover targets it would accept <br> ✅ The credential field sets a new one and never displays the current one <br> ✅ What would stop this channel sending is shown while the form is open, not after it is submitted <br> ✅ A rate card per channel with a calculator: a length and a destination, answered with the segments and the amount in the carrier's currency | P1 | 13 |
...
| NTF-BE-003 | Channel Transport, Receipts & Retry | 7 | NTF |
| NTF-FE-003 | Delivery Log & Channel Cost | 4 | NTF |
...
### EPIC ORD-BE-006: Renewals the Marketplace Does Not Own (Backend)
**Description**: The renewal run renewed everything, and most of it was never ours to renew. A subscription sold by a seller is renewed by that seller, who takes the money on their own system and reports it. Rolling their date on their behalf asserted renewals that may never have happened, and hid the only fact worth acting on — that nobody had heard from them.

| Story ID | Story Title | User Story | Acceptance Criteria | Priority | SP |
|----------|-------------|------------|---------------------|----------|----|
| ORD-BE-006-01 | Who Renews It **[P]** | As the **marketplace**, I want to renew only what I sell. | ✅ The split is `products.partner_id` — who sold it, not who fulfils it <br> ✅ The run charges and rolls only where the marketplace is the seller <br> ✅ A resold telco line is the seller's, because fulfilment and renewal are different questions <br> ✅ `roll_renewals` narrowed the same way — a repair we may make to our own book and not to a seller's | P0 | 8 |
| ORD-BE-006-02 | Awaiting a Seller **[P]** | As the **desk**, I want a seller who has gone quiet to be work rather than silence. | ✅ The run reports `awaiting` — every vendor-maintained cycle due with nothing reported <br> ✅ Each names the seller, the date and how late they are <br> ✅ A `renewal_watch` view banded watch / chase / escalate <br> ✅ A subscription that was never going to renew is not chased | P0 | 5 |
| ORD-BE-006-03 | Reporting a Renewal **[P]** | As a **seller**, I want to tell the marketplace I renewed one. | ✅ `report_renewal` is the only thing that moves a vendor-maintained date <br> ✅ One report, one cycle — several cycles behind means several reports, and the gap stays visible <br> ✅ The cycle out of order is refused; a cycle that has not started is refused <br> ✅ A repeat is **answered, not refused** — a retry from a seller's integration is not an error <br> ✅ The seller's own reference is required, so a query can be traced back to their record | P0 | 8 |
| ORD-BE-006-04 | Only Your Own **[P]** | As a **seller**, I want nobody else filing renewals against my listings, and I do not want the customer list. | ✅ A seller may report only against products they own <br> ✅ The operator may file on a seller's behalf, and the row says so <br> ✅ A customer may not report at all <br> ✅ `vendor_renewal_book` returns the subscription, never the subscriber — enforced by the database, not the screen | P0 | 5 |

### EPIC ORD-FE-004: Both Sides of a Renewal (Frontend)
**Description**: One subject, two desks. The marketplace plans its run and chases the sellers it is waiting on; the seller reports the cycles that are theirs.

| Story ID | Story Title | User Story | Acceptance Criteria | Priority | SP |
|----------|-------------|------------|---------------------|----------|----|
| ORD-FE-004-01 | The Renewals Desk **[P]** | As the **operator**, I want to see what a run will do before I press it. | ✅ What it will raise, what it will move, what it refuses and what is not ours <br> ✅ The chase list with the seller, the customer, the days late and the band <br> ✅ Record a seller's report taken by email or on a call | P0 | 8 |
| ORD-FE-004-02 | The Seller's Renewals **[P]** | As a **seller**, I want to report a renewal in one action. | ✅ My renewals in date order, with what is outstanding marked <br> ✅ One action on the cycle that is due, and none on one that is not <br> ✅ What I have already reported, with my own reference on it | P0 | 5 |
| ORD-FE-004-03 | What the Customer Is Told **[P]** | As a **customer**, I do not want to be told I am overdue when I am not. | ✅ A date gone by on a marketplace line reads overdue <br> ✅ On a seller's line it reads awaiting confirmation from the seller — the service is running and the gap is ours | P1 | 2 |
